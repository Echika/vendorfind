...
function loadVendorsTable() {
  ...
  const vendors = JSON.parse(
    localStorage.getItem("vendorfind_vendors") || "[]",
  );
  ...
  if (vendors.length === 0) {
    vendorTableBody.innerHTML =
      '<tr><td colspan="8" class="no-results">No vendors registered yet. <a href="signup.html">Register a vendor</a></td></tr>';
    return;
  }
...
function deleteVendor(vendorId) {
  if (
    confirm(
      "⚠️ Are you sure you want to delete this vendor? This action cannot be undone!",
    )
  ) {
    // Get all vendors
    const vendors = JSON.parse(
    ...
    const reviews = JSON.parse(
      localStorage.getItem("vendorfind_reviews") || "[]",
    );
